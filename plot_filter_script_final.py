...
parser.add_argument('--model_name', type=str, default='Gamma', choices=
                    ['Sinc', 'Sinc2', 'Gamma', 'Gauss', 'CNN'])
parser.add_argument('--model_path', type=str, default='IO/final/G_gamma/saved_model.pth')
parser.add_argument('--cfg_file', type=str, default='IO/final/G_gamma/KernelGamma_TIMIT.cfg')
parser.add_argument('--out_path', type=str, default='IO/final/imgs/G_gamma/')
...
model_paths = ['IO/final/cnn_new/1_saved_model.pth_best.pth',
               'IO/final/sinc_new/1_saved_model.pth_best.pth',
               'IO/final/sinc2_new/1_saved_model.pth_best.pth',
               'IO/final/gamma_new/1_saved_model.pth_best.pth',
               'IO/final/gauss_new/1_saved_model.pth_best.pth'
               ]
cfg_files = ['IO/final/cnn_new/KernelCNN_TIMIT.cfg',
             'IO/final/sinc_new/KernelSinc_TIMIT.cfg',
             'IO/final/sinc2_new/KernelSinc2_TIMIT.cfg',
             'IO/final/gamma_new/KernelGamma_TIMIT.cfg',
             'IO/final/gauss_new/KernelGauss_TIMIT.cfg'
             ]
filter_lengths = ['0',
                  '0',
                  '0',
...
plt.ylabel("#filters")
plt.legend()
plt.savefig(args.out_path + '_overal_hist.png', dpi=900)
plt.close()

# exit()

h_n_s, time_domain_filters, freq_domain_filters, phase_of_filters, freq_centers, f1_list, f2_list, amp_list = \
    SignalUtils.get_learned_filters_test(args.model_name, model, fs, N, num_scales)
...
for i in range(N_filt):
    ...
    plt.savefig(args.out_path + str(i) + '.png', dpi=600)
    plt.close()
...
for i in range(N_filt):
    ...
    plt.subplot(1, 3, 1)
    plt.plot(range(N), h_n_s[i, :], linewidth=0.5)
    plt.xlabel("Samples")
    plt.ylabel("h[n]")
    ...
    plt.savefig(args.out_path + str(i+1) + '.png', dpi=900)
    plt.close()
