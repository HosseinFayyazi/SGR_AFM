...
class SIDTrainer:
    ...
    def test(self, epoch, loss_tot, err_tot):
        ...
        if self.kernel_type == 'kernel_gauss_cascade':
            N = int(list(map(int, self.options['cnn_len_filt'].split(',')))[0])
            f1_list, f2_list = SignalUtils.get_gauss_cascade_learned_filters_test(self.CNN_net, self.fs, N)
            with open(self.save_path + '_' + str(epoch) + '.pth', 'wb') as filehandler1:
                pickle.dump([f1_list, f2_list], filehandler1)
